...
class ParserSystemCSV implements InterfaceParcerSystem {
    ...
    public function setAddParamsImport(array $data){
         ...
         if(count($tempData) > 0){
             throw  new Exception('Есть поля которые не совпадают  '.
                 implode(",",array_keys($tempData))." c",implode(",",$column_name));
         }
    ...
    public function setFile(string $filePath){
        if(file_exists($filePath) && strpos($filePath,self::FILE_TYPE)) {
    ...
    public function setTargetFiueldForUpdate(){
       ...
       try {
          ...
          $t = $list = explode($this->fileSeparator, trim($data));
           ...
           foreach ($list as $key => $item) {
               if(!empty($item)) {
                   $items = explode(',', $item);
                   foreach ($items as  $key__ =>$item__){
                       if(preg_match("/^\"(.+)\"$/",$item__)){
                           $items[$key__] =  substr($item__,1,strlen($item__)-2);
                       }
                   }
                   $newItems = [];
                   foreach ($takeIndex as $index__) {
