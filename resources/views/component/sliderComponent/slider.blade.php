@php
    ...
    $textComponet = (isset($admin))?"Add component Slider double click for set setting":"";
@endphp
@pushOnce('css-style')
{{--    <link type="text/css"  rel="stylesheet" href="/js/component/menuComponent/{{$listStyle[$style]}}">--}}
@endpushOnce
@pushOnce($stackNameScript)
<script src="/js/component/sliderComponent/slider.js"></script>
@endpushOnce

@if(!$include)
    <component id="component_slider_{{$name}}" data-name="{{$name}}">
    <div id="component_{{$name}}" class="slider"><span>{{$textComponet}}</span></div>
</component>
    ...
    <script date-id_script="{{$name}}">
        ...
        @endif
            var params_{{$name}} = {
                ...
                'paramsSlider':@php echo isset($paramsSlider)?json_encode($paramsSlider):"undefined"; @endphp,
            ...
            var component_{{$name}} = new Slider('#component_{{$name}}', params_{{$name}});
