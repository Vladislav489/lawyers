@php
    ...
    $textComponet = (isset($admin))?"Add component Cart double click for set setting":"";
@endphp
@pushOnce('css-style')
{{--    <link type="text/css"  rel="stylesheet" href="/js/component/menuComponent/{{$listStyle[$style]}}">--}}
@endpushOnce
@pushOnce($stackNameScript)
<script src="/js/component/boxComponent/box.js"></script>
@endpushOnce

@if(!$include)
    <component id="component_box_{{$name}}" data-name="{{$name}}">
    <div id="component_{{$name}}" class="box"><span>{{$textComponet}}</span></div>
</component>
    ...
    <script date-id_script="{{$name}}">
        ...
        @endif
            var params_{{$name}} = {
                ...
                'paramsBox':@php echo isset($paramsBox)?json_encode($paramsBox):"undefined"; @endphp,
            ...
            var component_{{$name}} = new Box('#component_{{$name}}', params_{{$name}});
