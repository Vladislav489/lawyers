...
@section('title', 'Контракт (редактирование)')
...
@section('content')
    <section class="mt-5">
        <div class="container">
            <div class="row">
                <div class="col-lg-6">
                    <h1 class="fs-2">Контракт «<span></span>» (редактирование)</h1>
    ...
    <script>
        ...
        fetch(`http://lawyers/mainstay/contract/getcontract?id=${params.id}`)
            .then((response) => response.json())
            .then((data) => {
                const url = `{{ route__('actionContractStore_contractmainstaycontroller') }}?id=${params.id}`;
