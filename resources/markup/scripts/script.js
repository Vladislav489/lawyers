const selectRow = (selectBlock, select = false) => {
    ...
    const closeWindows = () => {
        // [...document.querySelectorAll(`.${selectBlock.getAttribute('class')}`)].forEach(item => {
        //     item.querySelector('.select-window').classList.remove('_open')
        //     item.querySelector('.select-window').style.height = `0px`
        // });
...
class Popup {
    ...
    #popupInit() {
        ...
        if (this.#popup) {
            ...
            // this.#popupClose = [this.#popup.querySelector('.popup_shadow'), ...this.#popup.querySelectorAll('.step .popup-close')]
